...
@Component({
  ...
  standalone: true,
  encapsulation: ViewEncapsulation.None, // estilos aplican al overlay del drawer
  imports: [
    CommonModule,
    FormsModule,
    ButtonModule,
  ...
  template: `
    <section class="order-form">
      ...
      <p-drawer
        ...
        <!-- ↑ sin contentStyle; lo resolvemos con CSS del componente -->
  ...
  styles: [
    `
      ...
      /* === Drawer inferior === */
      /* El contenido del drawer en columna ocupando todo el alto */
      :host ::ng-deep .cart-bottom .p-drawer-content {
        display: flex;
        flex-direction: column;
        height: 100%;
        padding: 0;
...
export class OrderFormComponent implements AfterViewInit {
  ...
  /** Altura dinámica del drawer: desde el borde inferior de las categorías hasta el borde inferior de la ventana */
  drawerHeight = signal<string>('70vh');

  @HostListener('window:resize')
  onResize() {
    this.recalcDrawerHeight();
  }
  ...
  /** Calcula altura = viewport - bottom(categorías). Mínimo 360px para móviles chicos. */
  private recalcDrawerHeight() {
    try {
      const rect = this.catsRef?.nativeElement.getBoundingClientRect();
      if (rect) {
        const viewport = window.innerHeight || document.documentElement.clientHeight;
        const h = Math.max(400, viewport - rect.bottom);
        this.drawerHeight.set(`${h}px`);
      }
    } catch {}
  }
  ...
  toggleCart(v: boolean) {
    this.cartOpen = v;
    if (v) setTimeout(() => this.recalcDrawerHeight());
  }
